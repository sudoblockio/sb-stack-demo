...
```python
import asyncio
from sb_ws_client import WsClient

async def main():
    client = WsClient("wss://echo.websocket.org")
    await client.connect()
    
    response = await client.send("Hello, WebSocket!")
    print(f"Echo: {response.as_text()}")
...
```typescript
import { WsClient } from 'sb-ws-client';
...
async function main() {
    ...
    const response = await client.send('Hello, WebSocket!');
    console.log('Echo:', response.asText());
...
```python
import asyncio
from sb_ws_client import WsClient, LoggingMiddleware, LogLevel
...
async def main():
    ...
    client = WsClient("wss://echo.websocket.org", middlewares=[logging])
    await client.connect()
...
```typescript
import { WsClient, LoggingMiddleware, LogLevel } from 'sb-ws-client';
    ...
    async def receiver():
        while True:
            try:
                frame = await rx.recv()
                if frame.is_text():
                    print(f"Received: {frame.as_text()}")
            except Exception:
                break
...
async function main() {
    ...
    const rx = client.subscribe();
    
    const receiverTask = (async () => {
        while (true) {
            try {
                const frame = await rx.recv();
                if (frame.isText()) {
                    console.log('Received:', frame.asText());
                }
            } catch (error) {
                break;
            }
